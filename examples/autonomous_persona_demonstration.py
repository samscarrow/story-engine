...
class AutonomousPersonaDemonstration:
    ...
    async def setup_infrastructure(self):
        ...
        self.simulation_engine.register_callback(
            "completion_logger", self.log_simulation_completion
        )

        self.simulation_engine.register_callback(
            "template_tracker", self.track_template_usage
        )
    ...
    async def log_simulation_completion(self, result):
        ...
        logger.info(
            f"🎭 Simulation completed: {result.persona} at depth {result.depth}"
        )
        logger.info(
            f"   Status: {result.status.value}, Time: {result.execution_time:.2f}s"
        )
        if result.recursive_results:
            logger.info(
                f"   Triggered {len(result.recursive_results)} recursive simulations"
            )
    ...
    async def track_template_usage(self, result):
        ...
        if result.result_data and "template_used" in result.result_data:
            template_preview = (
                result.template_used[:100] + "..."
                if len(result.template_used) > 100
                else result.template_used
            )
    ...
    async def demonstrate_character_simulator_agent(self):
        ...
        modified_context["situation"] = (
            "The same plague crisis, but now a trusted advisor has been found taking bribes from merchants seeking exemptions from quarantine."
        )
        ...
        request_id_2 = await self.simulation_engine.submit_simulation(
            ...
            priority=SimulationPriority.HIGH,
            metadata={
                "demonstration": "template_adaptation",
                "parent_scenario": request_id,
            },
        )
    ...
    async def demonstrate_scene_architect_agent(self):
        ...
        await self.wait_and_show_results(
            recursive_request, "Recursive Scene Elaboration"
        )
    ...
    async def demonstrate_multi_persona_collaboration(self):
        ...
        analysis_request_id = await self.simulation_engine.submit_simulation(
            ...
            context_data={
                ...
                "content_to_analyze": {
                    ...
                    "character_development": (
                        character_result.result_data if character_result else {}
                    ),
                },
                "analysis_focus": [
                    "dramatic_effectiveness",
        ...
        logger.info("\n🌳 COLLABORATION TREE")
        collaboration_tree = await self.simulation_engine.get_simulation_tree(
            scene_request_id
        )
        ...
        if result:
            ...
            if result.result_data:
                ...
                if hasattr(primary_result, "text"):
                    preview = (
                        primary_result.text[:200] + "..."
                        if len(primary_result.text) > 200
                        else primary_result.text
                    )
                elif hasattr(primary_result, "content"):
                    preview = (
                        primary_result.content[:200] + "..."
                        if len(primary_result.content) > 200
                        else primary_result.content
                    )
                else:
    ...
    async def wait_for_completion(self, request_id: str, max_wait_seconds: int = 60):
        ...
        logger.warning(
            f"Simulation {request_id} did not complete within {max_wait_seconds} seconds"
        )
        return None
    ...
    def print_simulation_tree(self, tree_node: dict, indent: int = 0):
        ...
        if result:
            ...
            depth = result["depth"]
            logger.info(
                f"{prefix}├─ {sim_id[:8]}... ({persona}, depth={depth}, {status})"
            )
        else:
