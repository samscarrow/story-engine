...
except Exception:  # Fallback for environments where agents are not available or signatures differ
    class _DummyAgent:
...
class StoryProjectManager:
    ...
    async def create_character_profile(self, character_concept: Dict[str, Any]) -> Dict[str, Any]:
        """
        Create a detailed character profile using CHARACTER_SIMULATOR agent
        """
        logger.info(f"Creating character profile for: {character_concept.get('name', 'Unknown')}")
        
        context_data = {
            'character_concept': character_concept,
            'project_context': self.project_state['world_context'],
            'existing_characters': [c['name'] for c in self.project_state['characters']],
        ...
        if result['status'] == 'completed':
            ...
            logger.error(f"Character creation failed: {result.get('error', 'Unknown error')}")
    ...
    async def design_scene(self, scene_requirements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Designing scene: {scene_requirements.get('title', 'Untitled Scene')}")
        ...
        context_data = {
            ...
            'previous_scenes': self.project_state['scenes'][-3:],  # Last 3 scenes for context
        ...
        if result['status'] == 'completed':
            ...
            if result.get('spawned_simulations'):
                self.metrics['agent_collaborations'] += len(result['spawned_simulations'])
                logger.info(f"Scene design spawned {len(result['spawned_simulations'])} sub-simulations")
    ...
    async def enhance_dialogue(self, scene_id: str, character_ids: List[str]) -> Dict[str, Any]:
        """
        Enhance dialogue in a scene using DIALOGUE_COACH agent
        """
        logger.info(f"Enhancing dialogue for scene {scene_id} with characters: {character_ids}")
        
        # Find the scene
        scene = next((s for s in self.project_state['scenes'] if s['id'] == scene_id), None)
        if not scene:
            raise ValueError(f"Scene {scene_id} not found")
        
        # Get character data
        characters = [c for c in self.project_state['characters'] if c['id'] in character_ids]
        ...
        context_data = {
            ...
            'enhancement_goals': ['authenticity', 'character_voice', 'dramatic_tension']
        }
    ...
    async def analyze_narrative_structure(self) -> Dict[str, Any]:
        ...
        context_data = {
            ...
            'world_context': self.project_state['world_context'],
            'analysis_focus': ['pacing', 'character_arcs', 'thematic_coherence', 'dramatic_structure'],
    ...
    async def run_complex_story_development_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            workflow_results['characters_created'] = [c['name'] for c in main_characters]
            ...
            workflow_results['workflow_metrics'] = {
                ...
                'successful_rate': self.metrics['successful_simulations'] / max(1, self.metrics['total_simulations']),
    ...
    async def _run_world_building_stage(self) -> Dict[str, Any]:
        ...
        simulation_id = await self.simulation_engine.submit_simulation(
            persona=LLMPersona.WORLD_BUILDER,
            context_data=context_data
        )
    ...
    async def _run_character_creation_stage(self) -> List[Dict[str, Any]]:
        ...
        for concept in character_concepts:
            simulation_id = await self.simulation_engine.submit_simulation(
                persona=LLMPersona.CHARACTER_SIMULATOR,
                context_data={'character_concept': concept, 'world_context': self.project_state['world_context']},
                priority=SimulationPriority.HIGH
            )
    ...
    async def _run_scene_design_stage(self, characters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        scene_requirements = [
            ...
            },
            {
                ...
                'characters': [characters[0]['id'], characters[1]['id']],  # Pilate, Claudia
                'setting': 'Private chambers',
                'dramatic_beats': ['intimate_moment', 'warning_delivered', 'doubt_planted'],
            ...
            },
            {
                ...
                'dramatic_beats': ['questioning', 'truth_seeking', 'political_calculation'],
    ...
    async def _run_dialogue_enhancement_stage(self, scenes: List[Dict[str, Any]], characters: List[Dict[str, Any]]):
        """Dialogue enhancement stage - enhance all scenes in parallel"""
        
        enhancement_tasks = []
        for scene in scenes:
            # Determine which characters appear in this scene
            scene_character_ids = scene.get('character_ids', [characters[0]['id']])  # Default to Pilate
    ...
    def _extract_world_context_from_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
...
async def run_example_usage():
    ...
    try:
        ...
        workflow_results = {
            ...
            'characters_created': ['Pontius Pilate', 'Claudia Procula', 'Marcus Flavius'],
