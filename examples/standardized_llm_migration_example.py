...
class MigratedCharacterSimulationEngine:
    ...
    def __init__(self, standardized_interface: StandardizedLLMInterface):
        ...
        logger.info(
            "MigratedCharacterSimulationEngine initialized with standardized interface"
        )
...
class MigratedNarrativePipeline:
    ...
    async def analyze_scene_quality(
        self, scene_content: str, analysis_criteria: List[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        query = self.llm_interface.create_analysis_query(
            ...
            or ["dramatic_tension", "character_development", "pacing"],
...
async def demonstrate_migration():
    ...
    try:
        ...
        logger.info(
            f"Legacy response received: {bool(hasattr(legacy_result, 'text') or hasattr(legacy_result, 'content'))}"
        )
    ...
    logger.info(
        f"  Average response times: {performance.get('average_response_times', {})}"
    )
    ...
    try:
        ...
        logger.info(
            f"Batch processing: {len(successful_results)}/{len(batch_queries)} queries successful"
        )
