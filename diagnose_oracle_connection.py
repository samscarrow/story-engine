...
def test_connection_methods():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT 'Connected to ' || USER || ' on ' || SYS_CONTEXT('USERENV', 'DB_NAME') FROM DUAL")
...
def check_database_status_with_oci():
    ...
    try:
        ...
        result = os.system('oci db autonomous-database list --compartment-id ocid1.compartment.oc1..aaaaaaaasmfbe4prr46zz6dxbv7mu2abnoiomqjunbn5pmsfsiq2zhxavtwa 2>&1')
        if result != 0:
