...
class BreakingPointTester:
    ...
    async def test_template_creation_breaking_points(self):
            ...
            huge_context = {
                "character_concept": {
                    ...
                    "detailed_relationships": {
                        f"person_{i}": f"relationship_{i}" * 1000 for i in range(1000)
                    },
                }
            }
        ...
        try:
            ...
            agent = CharacterSimulatorAgent(
                self.mock_orchestrator, template_repository_path=readonly_dir
            )
        ...
        try:
            ...
            try:
                ...
                sim_b = await engine.submit_simulation(
                    persona=LLMPersona.SCENE_ARCHITECT, context_data=circular_context_b
                )
    ...
    async def test_agent_collaboration_deadlocks(self):
        ...
        try:
            ...
            try:
                ...
                task_a = asyncio.create_task(
                    char_agent.run_simulation(context_a.context_data, depth=0)
                )
                task_b = asyncio.create_task(
                    scene_agent.run_simulation(context_b.context_data, depth=0)
                )
            ...
            try:
                ...
                if modification_tasks:
                    try:
                        await asyncio.wait_for(
                            asyncio.gather(*modification_tasks), timeout=15.0
                        )
                    except asyncio.TimeoutError:
                        failures.append(
                            {
        ...
        logger.info(
            f"Collaboration deadlock tests complete. Found {len(failures)} issues."
        )
        ...
        try:
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    agent.run_simulation(context.context_data, depth=0), timeout=10.0
                )
                end_time = time.time()
            ...
            for i, malformed_response in enumerate(malformed_responses):
                try:
                    self.mock_orchestrator.generate_with_persona = AsyncMock(
                        return_value=Mock(
                            content=(
                                malformed_response.get("content")
                                if isinstance(malformed_response, dict)
                                else malformed_response
                            ),
                            metadata=(
                                malformed_response.get("metadata", {})
                                if isinstance(malformed_response, dict)
                                else {}
                            ),
                        )
                    ...
                    result = await agent.run_simulation(
                        {"character_concept": {"name": f"MalformedTest{i}"}}, depth=0
                    )
            ...
            try:
                result = await agent.run_simulation(
                    {"character_concept": {"name": "AuthTest"}}, depth=0
                )
    ...
    async def test_template_evolution_edge_cases(self):
        ...
        try:
            ...
            try:
                ...
                for feedback in contradictory_feedbacks:
                    try:
                        evolved_template = await agent.modify_template(
                            evolved_template, context, feedback["reason"]
                        )
                    except Exception as e:
                        failures.append(
                            {
                ...
                if (
                    len(evolved_template) < len(base_template) * 0.5
                    or len(evolved_template) > len(base_template) * 3
                ):
                    failures.append(
                        {
                            "test": "contradictory_feedback",
            ...
            try:
                ...
                if template_dir.exists():
                    ...
                    (template_dir / "incomplete.poml").write_text(
                        "<template><system>"
                    )  # Incomplete
    ...
    async def test_concurrent_access_race_conditions(self):
        ...
        try:
                ...
                for i, agent in enumerate(agents):
                    ...
                    task = asyncio.create_task(
                        agent.modify_template(
                            base_template, context, f"Modification {i}"
                        )
                    )
                    modification_tasks.append(task)
            ...
            try:
                ...
                if (
                    len(successful_results) < len(simulation_tasks) * 0.8
                    ...
                    async def create_file(agent_id):
                        try:
                            ...
                            if not Path(shared_file_path).exists():
                                ...
                                Path(shared_file_path).write_text(
                                    f"Created by agent {agent_id}"
                                )
    ...
    def generate_breaking_point_report(
        self, test_results: Dict, all_failures: List[Dict]
    ) -> Dict:
        """
        ...
        for failure in all_failures:
            ...
            if any(
                keyword in issue
                for keyword in ["deadlock", "corruption", "crash", "exception"]
            ):
                critical_failures.append(failure)
            elif any(
                keyword in issue for keyword in ["timeout", "memory", "performance"]
            ):
...
async def run_breaking_point_analysis():
    ...
    for test_name, result in report["test_results"].items():
        ...
        print(
            f"   {status_icon} {test_name}: {result['failure_count']} failures in {result['execution_time']:.2f}s"
        )
        ...
        if result["failures"] and result["failure_count"] > 0:
            ...
            for failure in result["failures"][:3]:  # Show top 3
                print(
                    f"        • {failure.get('test', 'unknown')}: {failure.get('issue', 'No description')}"
                )
    ...
    for failure in report["detailed_failures"]:
        if any(
            keyword in failure.get("issue", "").lower()
    ...
    if breaking_points:
        for bp in breaking_points[:5]:  # Show top 5 breaking points
            print(
                f"   🔴 {bp.get('test', 'Unknown')}: {bp.get('issue', 'Unknown issue')}"
            )
    else:
