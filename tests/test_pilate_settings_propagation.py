    ...
    async def generate(self, prompt: str, **kwargs):
        ...
        if 'json' in system_prompt:
            if 'dialogue' in system_prompt:
                return R('{"dialogue": [{"speaker": "Spy", "line": "Test dialogue", "tone": "neutral", "recipient": "All"}]}')
            if 'plot' in system_prompt:
                return R('{"structure_type": "three_act", "beats": [{"name": "Spy Beat", "description": "A test beat", "tension": 5, "purpose": "testing"}]}')
...
def test_pilate_profiles_and_prompts_propagate_with_poml():
    ...
    async def run():
        ...
        assert "Rate the following" in spy.prompts[-1] or "Story Content (Excerpt)" in spy.prompts[-1]
        ...
        assert "Focus area:" in spy.prompts[-2] or "Enhancement Focus:" in spy.prompts[-2]
