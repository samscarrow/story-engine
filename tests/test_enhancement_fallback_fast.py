...
def test_enhancement_fallback_stage1_failure():
    async def run():
        engine = OrchestratedStoryEngine(
            orchestrator=FailingOrchestrator(), use_poml=True
        )
        original = "A scene paragraph about Pilate in Jerusalem."
        out = await engine.enhance_content(
...
def test_enhancement_fallback_stage2_failure():
    async def run():
        engine = OrchestratedStoryEngine(
            orchestrator=TwoCallFailingOrchestrator(), use_poml=True
        )
        original = "A scene paragraph about Pilate in Jerusalem."
        out = await engine.enhance_content(
            original, {"evaluation_text": "ok"}, "pacing"
        )
        assert isinstance(out, str) and len(out) > 0
