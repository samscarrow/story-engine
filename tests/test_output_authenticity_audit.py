...
class OutputAuthenticityAuditor:
                ...
                if 'generate_with_persona' in content:
                    ...
                    if 'return Mock' in content or 'AsyncMock' in content:
                        self.add_evidence(
                            ...
                            severity="critical",
                            sample_data=self.extract_code_sample(content, 'generate_with_persona'),
                            confidence=0.95
                        )
            ...
            try:
                ...
                if 'generate_base_template' in content:
                    ...
                    if template_method_start != -1:
                        ...
                        method_content = self.extract_method_content(content, template_method_start)
                        ...
                        if '.read_text()' in method_content and 'generate' not in method_content.lower():
                            self.add_evidence(
                                location=f"{file_path}:generate_base_template", 
                ...
                if 'modify_template' in content:
                    ...
                    if modify_method_start != -1:
                        method_content = self.extract_method_content(content, modify_method_start)
        ...
        for file_path in agent_files:
            try:
                ...
                if 'def run_simulation' in content:
                    simulation_methods = re.finditer(r'def run_simulation.*?(?=def|\Z)', content, re.DOTALL)
                    ...
                    for match in simulation_methods:
                        ...
                        if 'if context_data.get(' in method_content:
                            ...
                            pass
                        elif 'return {' in method_content and 'await' not in method_content:
                ...
                if 'select_template' in content or 'choose_template' in content:
                    ...
                    template_selection_pattern = r'(select_template|choose_template).*?(?=def|\Z)'
                    matches = re.finditer(template_selection_pattern, content, re.DOTALL)
                    
                    for match in matches:
                        method_content = match.group(0)
                        if 'random' in method_content and 'context' not in method_content:
                            self.add_evidence(
    ...
    async def audit_simulation_result_validity(self):
        ...
        example_files = list(self.project_root.glob("**/example*.py")) + list(self.project_root.glob("**/demo*.py"))
            ...
            try:
                ...
                if 'workflow_results' in content:
                    ...
                    if results_start != -1:
                        ...
                        if "'stages_completed': [" in results_section and 'await' not in results_section:
                            self.add_evidence(
                ...
                if 'metrics' in content and 'total_simulations' in content:
                    ...
                    for match in matches:
                        ...
                        if 'success_rate.*0\.' in metrics_content or 'total_simulations.*\d+' in metrics_content:
                            # Check if metrics are static
                            if 'calculate' not in metrics_content and 'compute' not in metrics_content:
                                self.add_evidence(
                ...
                if 'await.*simulation' in content:
                    ...
                    for match in matches:
                        ...
                        if '{' in sim_line and '}' in sim_line and 'await' not in sim_line:
                            self.add_evidence(
                                location=f"{file_path}:{self.get_line_number(content, match.start())}",
    ...
    async def audit_performance_metrics_authenticity(self):
        ...
        metric_files = (list(self.project_root.glob("**/*orchestrat*.py")) + 
        ...
        for file_path in metric_files:
            try:
                ...
                if 'get_performance' in content or 'performance_summary' in content:
                    perf_methods = re.finditer(r'def.*performance.*?(?=def|\Z)', content, re.DOTALL)
                    ...
                    for match in perf_methods:
                        ...
                        if 'return {' in method_content:
                            ...
                            has_computation = any(op in method_content for op in ['len(', 'sum(', 'count', 'time()', 'statistics'])
                            has_tracking = any(track in method_content for track in ['self.metrics', 'self.stats', 'self.counters'])
                    ...
                    if 'time.time()' not in content and 'perf_counter()' not in content:
                        if 'response_time.*=' in content:
                            self.add_evidence(
                                ...
                                severity="medium",
                                sample_data=self.extract_code_sample(content, 'response_time'),
                                confidence=0.75
                            )
                ...
                if 'success_rate' in content:
                    ...
                    for match in matches:
                        ...
                        if ('0.' in success_line or '1.0' in success_line) and '/' not in success_line:
                            self.add_evidence(
                                location=f"{file_path}:{self.get_line_number(content, match.start())}",
    ...
    async def audit_error_handling_genuineness(self):
        ...
        for file_path in all_python_files:
            try:
                ...
                except_blocks = re.finditer(r'except.*?:.*?(?=except|finally|def|class|\Z)', content, re.DOTALL)
                ...
                if 'fallback' in content:
                    ...
                    for match in matches:
                        ...
                        if any(word in fallback_line.lower() for word in ['mock', 'default', 'placeholder']):
                            self.add_evidence(
                                location=f"{file_path}:{self.get_line_number(content, match.start())}",
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            self.add_evidence(
                                ...
                                severity="medium" if category in ['suspicious_consistency', 'generic_responses'] else "high",
                                sample_data=match.group(0),
                                confidence=0.7
                            )
    ...
    def add_evidence(self, location: str, evidence_type: str, description: str, 
                    severity: str, sample_data: str, confidence: float):
    ...
    def extract_code_sample(self, content: str, keyword: str, context_lines: int = 2) -> str:
    ...
    def generate_authenticity_report(self) -> Dict[str, Any]:
        ...
        authenticity_score = max(0, 100 - (weighted_score / max(1, max_possible_score) * 100))
        
        # Identify top concerns
        top_concerns = sorted(self.evidence, key=lambda e: (severity_weights[e.severity], e.confidence), reverse=True)[:10]
        ...
        report = {
            'summary': {
                ...
                'pipeline_status': self.determine_pipeline_status(authenticity_score, critical_evidence)
            },
            'evidence_breakdown': {
                ...
                'non_functional_outputs': len(self.audit_results['non_functional_outputs']),
    ...
    def determine_pipeline_status(self, authenticity_score: float, critical_evidence: List) -> str:
...
async def run_output_authenticity_audit(project_root: str = ".") -> Dict[str, Any]:
    ...
    for i, concern in enumerate(report['top_concerns'][:5], 1):
        severity_icon = {"critical": "🔴", "high": "🟡", "medium": "🟠", "low": "🟢"}.get(concern['severity'], "⚪")
        print(f"   {i}. {severity_icon} [{concern['severity'].upper()}] {concern['description']}")
    ...
    for rec in report['recommendations']:
        ...
        print(f"   {priority_icon} [{rec['priority']}] {rec['issue']} ({rec['count']} issues)")
