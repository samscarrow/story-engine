...
# Import POML engine
from poml.lib.poml_integration import POMLEngine, StoryEnginePOMLAdapter
...
class POMLCharacterSimulationEngine:
    ...
    def get_simulation_prompt(self, character: CharacterState, 
                            situation: str, 
        ...
        if context:
            context_prompt = self.poml.render(
                'components/context_extension.poml',
                {'context': context}
            )
            prompt = f"{prompt}\n\n{context_prompt}"
            ...
            try:
                ...
                result = {
                    ...
                    "metadata": {
                        **response.metadata,
                        "used_poml": use_poml,
                        "template": "character_response.poml" if use_poml else "string_based"
                    },
    ...
    def _validate_response_schema(self, response_data: Dict):
        ...
        try:
            ...
            import re
            schema_match = re.search(r'<json-schema>(.*?)</json-schema>', 
                                    schema_template, re.DOTALL)
    ...
    async def run_narrative_sequence(self,
                                   character: CharacterState,
                                   story_beats: List[Dict],
                                   use_poml: bool = True) -> List[Dict]:
        """
            ...
            if use_poml:
                scene_prompt = self.poml.render(
                    'templates/narrative/scene_crafting.poml',
                    {
                        ...
                        'previous_context': results[-1]['response']['dialogue'] if results else ""
                    }
                )
    ...
    async def generate_character_variant(self,
                                        base_character: CharacterState,
                                        variant_type: str) -> CharacterState:
        """
    ...
    def get_available_templates(self) -> Dict[str, List[str]]:
        ...
        for category in templates.keys():
            ...
            if category_path.exists():
                templates[category] = [
                    f.name for f in category_path.glob('*.poml')
                ]
        
        return templates
    
    async def benchmark_poml_performance(self,
                                        character: CharacterState,
...
def create_poml_engine(llm_client, config: Dict = None) -> POMLCharacterSimulationEngine:
    """
...
class SimulationEngineMigrator:
    ...
    @staticmethod
    async def compare_outputs(engine: POMLCharacterSimulationEngine,
                             character: CharacterState,
                             situation: str) -> Dict:
        """
    ...
    async def main():
        ...
        character = CharacterState(
            ...
            emotional_state=EmotionalState(
                anger=0.3, doubt=0.5, fear=0.2, 
                compassion=0.6, confidence=0.7
            ),
