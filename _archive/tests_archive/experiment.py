...
async def main():
    ...
    print(f"😰 Current emotional state: Doubt={pilate.emotional_state.doubt:.1f}, Fear={pilate.emotional_state.fear:.1f}")
    ...
    for emphasis in emphases:
        ...
        if 'emotional_shift' in response:
            ...
            shift_summary = ", ".join([f"{k}:{v:+.1f}" for k, v in shifts.items() if v != 0])
            if shift_summary:
    ...
    emotional_variants = [
        ("Angry Pilate", EmotionalState(anger=0.9, doubt=0.3, fear=0.2, compassion=0.1, confidence=0.8)),
        ("Fearful Pilate", EmotionalState(anger=0.1, doubt=0.8, fear=0.9, compassion=0.4, confidence=0.2)),
        ("Compassionate Pilate", EmotionalState(anger=0.2, doubt=0.5, fear=0.3, compassion=0.9, confidence=0.6))
    ]
    
    situation_short = "The prisoner stands before you. The crowd shouts for blood. You must choose."
    
    for variant_name, emotion_state in emotional_variants:
        print(f"\n🎭 {variant_name}:")
        print(f"   Anger={emotion_state.anger:.1f}, Doubt={emotion_state.doubt:.1f}, Fear={emotion_state.fear:.1f}, Compassion={emotion_state.compassion:.1f}")
