...
class GemmaEmotionalTracker:
    ...
    def record_scene(self, scene_name: str, character_state: CharacterState, 
                     response: dict, elapsed_time: float):
    ...
    def display_journey(self, model_name: str = "Gemma-3-12b"):
        ...
        # Header
        print(f"{'Scene':<25} {'Anger':<8} {'Doubt':<8} {'Fear':<8} {'Comp':<8} {'Conf':<8}")
        print("-" * 80)
        
        # Initial state (if exists)
        if self.journey:
            first = self.journey[0]['emotions']
            print(f"{'Initial State':<25} {first['anger']:>6.2f}  {first['doubt']:>6.2f}  "
        ...
        if len(self.journey) > 1:
            ...
            sorted_changes = sorted(total_changes.items(), 
...
async def run_gemma_emotional_sequence():
    ...
    engine = SimulationEngine(
        llm_provider=llm,
        max_concurrent=1  # Sequential for clarity
    )
    ...
    pilate = CharacterState(
        ...
        fears=["rebellion", "losing position", "Caesar's displeasure", "divine judgment"],
    ...
    print(f"📍 Starting emotional state: Confidence={pilate.emotional_state.confidence:.2f}, "
    ...
    for i, scene in enumerate(scenes, 1):
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                print(f"🤔 Thinks: {thought[:150]}{'...' if len(thought) > 150 else ''}")
                    ...
                    if isinstance(shifts, dict):
                        ...
                        for emotion, change in shifts.items():
                            if isinstance(change, (int, float)) and abs(change) > 0.05:
                                ...
                                significant_shifts.append(f"{emotion}{direction}{abs(change):.2f}")
                        
                        if significant_shifts:
                            print(f"📊 Emotional shifts: {' | '.join(significant_shifts)}")
                
                # Track the scene
                tracker.record_scene(scene['name'], pilate, response, elapsed)
            else:
                print(f"📝 Raw response: {str(response)[:300]}")
                tracker.record_scene(scene['name'], pilate, {'dialogue': str(response)}, elapsed)
...
async def compare_gemma_vs_mock():
    ...
    test_situation = "Jesus stands silent before you. The crowd demands action. What do you do?"
    
    # Test both models
    models = []
    
    # Try Gemma
    gemma_llm = LMStudioLLM(endpoint="http://localhost:1234/v1", model="google/gemma-3-12b")
