...
async def test_llm_connection():
    ...
    try:
        ...
        if is_available:
            ...
            print(f"📝 Test response received (length: {len(test_response.content)} chars)")
            ...
            try:
                ...
                print("📝 Raw response:", test_response.content[:200] + "..." if len(test_response.content) > 200 else test_response.content)
    ...
    class SequenceMockLLM(MockLLM):
        ...
        def _generate_contextual_response(self, emotions, emphasis, situation, temperature):
            ...
            if "trial" in situation.lower() or "judge" in situation.lower():
                if emphasis == "power" and anger_level > 0.7:
                    return {
                        ...
                        "emotional_shift": {"anger": 0.1, "confidence": 0.15, "fear": -0.05}
                    }
                    ...
                    return {
                        ...
                        "emotional_shift": {"doubt": 0.1, "fear": 0.05, "compassion": 0.05}
                    }
                elif fear_level > 0.8:
                    return {
                        ...
                        "emotional_shift": {"fear": 0.15, "doubt": 0.1, "confidence": -0.1}
                    }
                    ...
                    return {
                        ...
                        "emotional_shift": {"compassion": 0.1, "doubt": 0.1, "anger": -0.05}
                    }
...
async def run_emotional_sequence_experiment():
    ...
    engine = SimulationEngine(
        llm_provider=llm,
        max_concurrent=1  # Sequential for better story flow
    )
    ...
    pilate = CharacterState(
        ...
        traits=["pragmatic", "politically-astute", "duty-bound", "increasingly-conflicted"],
    ...
    for i, scene in enumerate(story_sequence, 1):
        ...
        # Run simulation
        print("⏳ Running simulation with real LLM..." if is_real_llm else "⏳ Running simulation...")
        ...
        try:
            ...
            print(f"😰 Current emotions: A:{emotions.anger:.2f} D:{emotions.doubt:.2f} F:{emotions.fear:.2f} C:{emotions.compassion:.2f} Conf:{emotions.confidence:.2f}")
