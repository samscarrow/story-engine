...
class CacheManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized CacheManager: max_size={max_size_mb}MB, ttl={default_ttl}s"
        )

    def _generate_key(
        self, prompt: str, temperature: float, model: Optional[str] = None
    ) -> str:
...
class SimulationCache(CacheManager):
    ...
    async def get_simulation(
        self, character_id: str, situation: str, emphasis: str, temperature: float
    ) -> Optional[Dict]:
    ...
    async def set_simulation(
        ...
        success = await self.set(
            prompt, temperature, response, model="simulation", ttl=ttl
        )
    ...
    async def get_similar_simulations(
        self, character_id: str, situation: str, threshold: float = 0.8
    ) -> list:
...
async def cache_maintenance_task(cache: CacheManager, interval: int = 300):
    ...
    while True:
        ...
        if stats:
            logger.info(
                f"Cache stats: hit_rate={stats['hit_rate']:.2%}, size={stats['cache_size']}"
            )
