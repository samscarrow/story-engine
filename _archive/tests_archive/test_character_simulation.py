...
class TestEmotionalState(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.high_emotion_state = EmotionalState(
            anger=0.9, doubt=0.8, fear=0.9, compassion=0.2, confidence=0.1
        )
...
class TestCharacterState(unittest.TestCase):
    ...
    def test_simulation_prompt_generation(self):
        ...
        prompt = self.valid_character.get_simulation_prompt("Test situation", "power")
    ...
    def test_mock_llm_response(self):
        ...
        async def test():
            ...
            response = await llm.generate_response(
                "Test prompt with power", temperature=1.0
            )
            ...
            # Test doubt emphasis
            response = await llm.generate_response(
                "Test prompt with doubt", temperature=0.7
            )
            data = json.loads(response.content)
...
class TestSimulationEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.engine = SimulationEngine(llm_provider=self.mock_llm, max_concurrent=2)
    ...
    def test_single_simulation(self):
        ...
        async def test():
            result = await self.engine.run_simulation(
                self.test_character, "Test situation", "doubt", 0.8
            )
    ...
    def test_emotional_shift_application(self):
        ...
        async def test():
            ...
            with patch.object(self.mock_llm, "generate_response") as mock_gen:
                ...
                await self.engine.run_simulation(self.test_character, "Test", "neutral")
    ...
    def test_concurrent_simulation_limit(self):
        ...
        async def test():
            ...
            engine = SimulationEngine(llm_provider=self.mock_llm, max_concurrent=2)
            ...
            with patch.object(self.mock_llm, "generate_response", tracked_generate):
                results = await engine.run_multiple_simulations(
                    self.test_character, "Test", num_runs=5
                )
            ...
            with patch.object(
                ...
                with self.assertRaises(SimulationError):
                    await self.engine.run_simulation(
                        self.test_character, "Test", "neutral"
                    )
    ...
    def test_full_simulation_workflow(self):
        ...
        async def test():
            ...
            pilate = CharacterState(
                ...
                fears=["rebellion"],
                desires=["peace"],
                emotional_state=EmotionalState(anger=0.4, doubt=0.7, fear=0.6),
                memory=CharacterMemory(),
                current_goal="Maintain order",
            )

            # 2. Initialize engine
            engine = SimulationEngine(llm_provider=MockLLM(), max_concurrent=3)

            # 3. Run simulations
            results = await engine.run_multiple_simulations(
                pilate, "The crowd demands justice", num_runs=3
            )
...
class TestPerformance(unittest.TestCase):
    ...
    def test_concurrent_load(self):
        ...
        async def test():
            engine = SimulationEngine(llm_provider=MockLLM(), max_concurrent=10)
