...
class QuickEnhancementTest:
    ...
    async def enhance_scene(
        self, scene: TestScene, focus: str = "conflict"
    ) -> TestScene:
        """Enhance a scene"""
    ...
    async def run_enhancement_test(self):
        ...
        for scene in test_scenes:
            ...
            if enhanced_scene.situation != scene.situation:
                ...
                print(
                    f"   {enhanced_scene.situation[:300]}{'...' if len(enhanced_scene.situation) > 300 else ''}"
                )
        ...
        if self.enhancement_history:
            ...
            avg_time = sum(h["time"] for h in self.enhancement_history) / len(
                self.enhancement_history
            )
            ...
            for h in self.enhancement_history:
                ...
                print(
                    f"  {symbol} {h['scene']}: {h['original_score']} → {h['enhanced_score']} (focus: {h['focus']})"
                )
...
async def test_iterative_enhancement():
    ...
    for i in range(3):
        ...
        if enhanced.situation != current_scene.situation:
            ...
            print(
                f"Result: {eval['score']} → {new_eval['score']} ({'+' if improvement >= 0 else ''}{improvement})"
            )
