...
class DramaticMockLLM(MockLLM):
    ...
    async def generate_response(self, prompt, temperature=0.7, max_tokens=500):
        ...
        return LLMResponse(
            ...
            {"model": "dramatic_mock", "temperature": temperature},
        )
    ...
    def _generate_dramatic_response(
        self, emotions, emphasis, situation, scene_num, temperature
    ):
...
def print_dramatic_emotional_visualization(journey_history):
    ...
    # Print header
    print(
        f"{'Event':<25} {'Anger':<8} {'Doubt':<8} {'Fear':<8} {'Comp':<8} {'Conf':<8}"
    )
    print("-" * 80)
    ...
    for i, state in enumerate(journey_history):
        ...
        print(
            f"{event_name:<25} {values[0]:<8} {values[1]:<8} {values[2]:<8} {values[3]:<8} {values[4]:<8}"
        )

    print("\n🎯 EMOTIONAL INTENSITY LEGEND:")
    print(
        "🔥 = Extreme (0.8+)  🌡️ = High (0.6+)  💧 = Medium (0.4+)  ❄️ = Low (0.2+)  🧊 = Minimal (<0.2)"
    )
    ...
    for emotion in emotions:
        ...
        max_scene = next(
            state for state in journey_history if state[emotion] == max_val
        )
...
async def run_dramatic_experiment():
    ...
    print(
        f"⚡ Initial state: Confident administrator (Conf: {pilate.emotional_state.confidence:.2f})"
    )
    print()
    ...
    for i, scene in enumerate(dramatic_sequence, 1):
        ...
        try:
            ...
            print(
                "⏳ Generating response..."
                + (" (Real LLM)" if using_real_llm else " (Dramatic Mock)")
            )
    ...
    print(
        f"🤖 LLM used: {'Real LMStudio' if using_real_llm else 'Enhanced Dramatic Mock'}"
    )
    print(
        "🎪 Final emotional state: Complex mix of guilt, compassion, and broken confidence"
    )
