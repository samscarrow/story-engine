        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                try:
                    ...
                    required_fields = ['dialogue', 'thought', 'action', 'emotional_shift']
...
async def run_gemma_emotional_test():
    ...
    engine = SimulationEngine(
        llm_provider=llm,
        max_concurrent=1  # Run sequentially for Gemma
    )
    ...
    print(f"📍 Initial emotional state: Confidence={pilate.emotional_state.confidence:.1f}, Doubt={pilate.emotional_state.doubt:.1f}")
    print()
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                print(f"💬 Pilate: \"{dialogue[:150]}...\"" if len(dialogue) > 150 else f"💬 Pilate: \"{dialogue}\"")
                ...
                if 'emotional_shift' in response:
                    ...
                    if isinstance(shifts, dict):
                        shift_str = ", ".join([f"{k}:{v:+.2f}" for k, v in shifts.items() if v != 0])
                        if shift_str:
            ...
            print(f"😰 Current state: Doubt={pilate.emotional_state.doubt:.2f}, Fear={pilate.emotional_state.fear:.2f}, Confidence={pilate.emotional_state.confidence:.2f}")
    ...
    if results_comparison:
        ...
        avg_length = sum(r['response_length'] for r in results_comparison) / len(results_comparison)
