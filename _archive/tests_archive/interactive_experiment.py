...
class InteractiveMockLLM(MockLLM):
    ...
    async def generate_response(
        ...
        template = self.response_templates.get(
            template_key, self._default_response(emotions, emphasis)
        )
        ...
        return LLMResponse(
            content, {"model": "interactive_mock", "temperature": temperature}
        )
    ...
    def _parse_emotions(self, prompt):
        ...
        for emotion in ["anger", "doubt", "fear", "compassion", "confidence"]:
            if (
                f"{emotion.title()}: 0.9" in prompt
                or f"{emotion.title()}: 0.8" in prompt
            ):
                emotions[emotion] = "high"
            elif (
                f"{emotion.title()}: 0.1" in prompt
                or f"{emotion.title()}: 0.2" in prompt
            ):
    ...
    def _add_dramatic_flair(self, template):
        ...
        template["dialogue"] = (
            template["dialogue"].upper()
            if len(template["dialogue"]) < 50
            else template["dialogue"] + "!"
        )
...
async def character_builder():
    ...
    if choice in ["1", "2", "3"]:
        ...
        character = CharacterState(
            ...
            emotional_state=EmotionalState(
                anger=0.4, doubt=0.7, fear=0.5, compassion=0.3, confidence=0.4
            ),
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        for emphasis in emphases:
            result = await engine.run_simulation(
                character, scenario, emphasis=emphasis, temperature=0.8
            )
        ...
        print(
            f"  😰 Current emotions: Doubt={emotions.doubt:.2f}, Fear={emotions.fear:.2f}, Anger={emotions.anger:.2f}"
        )
    ...
    extreme_states = [
        (
            "Enraged",
            EmotionalState(
                anger=0.95, doubt=0.2, fear=0.3, compassion=0.1, confidence=0.8
            ),
        ),
        (
            "Terrified",
            EmotionalState(
                anger=0.2, doubt=0.8, fear=0.95, compassion=0.3, confidence=0.1
            ),
        ),
        (
            "Utterly Lost",
            EmotionalState(
                anger=0.3, doubt=0.95, fear=0.7, compassion=0.2, confidence=0.1
            ),
        ),
        (
            "Deeply Moved",
            EmotionalState(
                anger=0.1, doubt=0.4, fear=0.3, compassion=0.95, confidence=0.6
            ),
        ),
    ]

    test_situation = (
        "The moment of ultimate decision has arrived. All eyes are upon you."
    )
