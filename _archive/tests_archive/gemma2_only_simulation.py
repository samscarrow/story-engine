...
class Gemma2Simulation:
    ...
    async def call_gemma2(self, system_prompt: str, user_prompt: str, temperature: float = 0.7):
...
{{"dialogue": "what you say", "thought": "inner thoughts", "action": "physical action", "emotional_shift": {{"doubt": 0.1, "fear": 0.1, "confidence": -0.1}}}}"""
        ...
        try:
            result = await self.call_gemma2(system_prompt, scene['situation'], scene.get('temperature', 0.7))
            ...
            if data:
                ...
                print(f"🤔 *{data.get('thought', 'N/A')[:100]}{'...' if len(data.get('thought', '')) > 100 else ''}*")
                print(f"🎬 {data.get('action', 'N/A')[:80]}{'...' if len(data.get('action', '')) > 80 else ''}")
                ...
                if 'emotional_shift' in data and isinstance(data['emotional_shift'], dict):
                    ...
                    for emotion, change in shifts.items():
                        if emotion in current_emotions and isinstance(change, (int, float)):
                            ...
                            if abs(change) > 0.05:
                                ...
                                print(f"  {emotion}: {old_value:.2f} {arrow} {new_value:.2f}")
    ...
    def display_journey(self):
        ...
        for state in self.emotional_states:
            print(f"{state['scene']:<25} {state['doubt']:>8.2f} {state['fear']:>8.2f} {state['confidence']:>12.2f}")
        ...
        if len(self.emotional_states) > 1:
            ...
            print("\n" + "-" * 60)
            print("TOTAL CHANGES:")
            print(f"  Doubt: {first['doubt']:.2f} → {last['doubt']:.2f} ({last['doubt']-first['doubt']:+.2f})")
            print(f"  Fear: {first['fear']:.2f} → {last['fear']:.2f} ({last['fear']-first['fear']:+.2f})")
            print(f"  Confidence: {first['confidence']:.2f} → {last['confidence']:.2f} ({last['confidence']-first['confidence']:+.2f})")
