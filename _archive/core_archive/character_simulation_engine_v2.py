...
class SimulationEngine:
    """Production-ready simulation engine with LLM integration and caching"""
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized SimulationEngine with {llm_provider.__class__.__name__}")
    ...
    async def run_simulation(self, 
        ...
        async with self.semaphore:  # Limit concurrent simulations
            try:
                ...
                # Parse response - with structured output this is simple
                try:
                    response_data = json.loads(response.content)
                    logger.info("Parsed structured response successfully")
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse structured response: {e}")
                    # Structured output should never fail, but have a fallback
                    response_data = {
    ...
    async def run_multiple_simulations(self,
                                      ...
                                      situation: str,
                                      num_runs: int = 10,
                                      emphases: Optional[List[str]] = None) -> List[Dict]:
        ...
        for i in range(num_runs):
            ...
            task = self.run_simulation(char_copy, situation, emphasis, temp)
            tasks.append(task)
