...
MODULES = [
    ...
    (
        "core/orchestration",
        "Python",
        "LLM orchestration, agent controllers, standardized interfaces",
    ),
    (
        "core/story_engine",
        "Python",
        "narrative graph, pipelines, world state, story arcs",
    ),
    (
        "core/character_engine",
        "Python",
        "character simulation engines and group dynamics",
    ),
            ...
            "largest": ", ".join(
                sorted(
                    [
                        os.path.basename(x)
                        for x in sorted(
                            files, key=lambda p: os.path.getsize(p), reverse=True
                        )[:3]
                    ]
                )
            ),
        ...
        modules.append(
            {
                ...
                "env_vars": sorted(
                    {
                        k
                        for k, files in env_map.items()
                        if any(which_module(fp) == name for fp in files)
                    }
                ),
            }
        )
...
""".strip()
    ...
    for m in modules:
        ...
        internals = ", ".join(
            sorted([d for d in m["internal_deps"] if d != m["name"]])[:6]
        )
        externals = ", ".join(sorted(m["external_deps"])[:6])
        envs = ", ".join(sorted(m["env_vars"])[:8])
        tests = "yes" if m["name"] == "tests" else "partial"
        notes = ""
        if m["name"] == "core/storage":
            notes = "DB drivers; wallet/SSL may be required"
        if m["name"] == "core/story_engine":
            notes = (
                notes + "; " if notes else ""
            ) + "High centrality; watch for cycles"
        if m["name"] == "core/orchestration":
            notes = (notes + "; " if notes else "") + "External LLM APIs; timeouts"
        rows.append(
            "|".join(
                [
                    m["name"],
    ...
    meta = {
        ...
        "limits": {"max_file_size_bytes": MAX_BYTES_DEFAULT},
    }
        ...
        "modules": [
            {
                ...
                "path": m["path"],
                "language": m["language"],
                "public_api": sorted(m["public_api"]),
                "internal_deps": sorted(m["internal_deps"]),
                "external_deps": sorted(m["external_deps"]),
                "env_vars": sorted(m["env_vars"]),
            }
            for m in modules
        ],
        "edges": edges_json,
    ...
    base = pathlib.Path("DOCS/modules")
...
""".strip()
        with open(d / "SUMMARY.md", "w", encoding="utf-8") as f:
            f.write(content)
...
def main():
    ...
    ap.add_argument(
        "--suffix",
        default="",
        help="Output suffix (e.g., L4) to avoid overwriting existing docs",
    )
    ...
    suffix = args.suffix
    if not suffix and (
        os.path.exists("DOCS/REPO_MAP.md") or os.path.exists("DOCS/MODULES.md")
    ):
        suffix = "L4"
