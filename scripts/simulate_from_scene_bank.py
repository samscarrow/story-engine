...
async def simulate_from_scene(scene: Dict[str, Any], character_flags: dict | None = None) -> Dict[str, Any]:
    ...
    profiles[StoryComponent.SCENE_DETAILS]["max_tokens"] = max(1200, profiles[StoryComponent.SCENE_DETAILS]["max_tokens"])  # noqa: E501
    profiles[StoryComponent.CHARACTER_DIALOGUE]["max_tokens"] = max(600, profiles[StoryComponent.CHARACTER_DIALOGUE]["max_tokens"])  # noqa: E501
    ...
    plot_point = {"name": scene.get("title", "Scene"), "purpose": "Use pre-authored scene context", "tension": 6}
    ...
    # Dialogue for Pilate
    dialogue = await engine.generate_dialogue(sim_scene, characters[0], "Continue the exchange")
