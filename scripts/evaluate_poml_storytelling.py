...
def load_character(character_id: str) -> dict:
    ...
    char_path = Path(__file__).resolve().parents[1] / f"poml/config/characters/{character_id}.yaml"
...
async def evaluate_poml_model(model_identifier: str, character_id: str, emphasis: str, sync_mode: bool, model_config: dict = None):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a storytelling model using the POML framework.")
    
    # Add standardized model/client arguments
    add_model_client_args(parser)
    
    parser.add_argument("--character", default="centurion", help="The character ID to use.")
    parser.add_argument("--emphasis", default="pragmatic", help="The emphasis mode for the response.")
    parser.add_argument("--sync", action="store_true", help="Force update of the script's last-used model state.")
    ...
    asyncio.run(evaluate_poml_model(model_to_use, args.character, args.emphasis, args.sync, model_config))
