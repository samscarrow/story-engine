...
from story_engine.core.character_engine.meta_narrative_pipeline import (
...
async def run(
    ...
    *,
    ...
    load_dotenv_keys()
    pipeline = MetaNarrativePipeline(
        use_poml=True,
    ...
    review_round1 = await pipeline.review_throughlines(
        char_dict, situations, sims_round1
    )
    ...
    if recommended:
        ...
        review_round2 = await pipeline.review_throughlines(
            char_dict, situations, sims_round1 + sims_round2
        )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Run meta-narrative pipeline over character simulations"
    )
    ...
    p.add_argument(
        "--store-db",
        action="store_true",
        help="Store the final result JSON into PostgreSQL using DB_* env vars",
    )
    p.add_argument(
        "--workflow-name",
        default="meta_pipeline",
        help="Workflow name key under which to store the output",
    )
    ...
    p.add_argument(
        ...
        nargs="*",
        default=[],
        help="Bias toward these metrics (e.g., Pacing Dialogue Structure)",
    )
    p.add_argument(
        "--weights",
        default="",
        help="Comma-separated weights, e.g., 'Pacing=2,Dialogue=1.5,Structure=1'",
    )
    ...
    p.add_argument(
        ...
        action="append",
        default=[],
        help="Per-character runtime flags, format id:key=value; can be repeated",
    )
