...
async def create_character_persona(model_identifier: str, concept: str, character_id: str):
    ...
    try:
        ...
        freeform_system_prompt = adapter.engine.render_roles(
            'characters/persona_generation_freeform.poml',
            {'concept': concept}
        )
        ...
        structured_response = await orchestrator.generate(
            ...
            temperature=0.1,
            max_tokens=2000,
            response_format={"type": "json_object"} # Assuming YAML can be represented as JSON
        )
        ...
        try:
            ...
            output_path = Path(__file__).resolve().parents[1] / f"poml/config/characters/{character_id}.yaml"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create a character persona using a two-stage pipeline.")
    parser.add_argument("--model", required=True, help="The model identifier to use.")
    parser.add_argument("--concept", required=True, help="A brief concept for the character.")
    parser.add_argument("--id", required=True, help="The character ID to use for the output file (e.g., 'caiaphas').")
