...
async def main():
    parser = argparse.ArgumentParser(
        description="Run iterative persona simulation and store results"
    )
    ...
    parser.add_argument(
        "--character-id",
        default="pontius_pilate",
        help="Character ID matching a persona YAML filename",
    )
    parser.add_argument(
        "--situation",
        required=False,
        default="The crowd demands a decision; tensions are high in the Praetorium.",
        help="Situation prompt",
    )
    parser.add_argument("--emphasis", default="duty", help="Emphasis mode")
    parser.add_argument(
        "--iterations", type=int, default=3, help="Number of iterative attempts"
    )
    parser.add_argument(
        "--window", type=int, default=3, help="Number of previous attempts to include"
    )
    parser.add_argument(
        "--workflow-name", default="iterative_persona_sim", help="Storage workflow name"
    )
    ...
    try:
        ...
        print(
            json.dumps(
                {"status": "ok", "stored": True, "workflow": args.workflow_name},
                indent=2,
            )
        )
    finally:
