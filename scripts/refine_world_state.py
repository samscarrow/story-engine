...
async def main():
    parser = argparse.ArgumentParser(
        description="Refine world state via LMStudio and save the result"
    )
    ...
    parser.add_argument(
        "--characters", nargs="*", default=[], help="Focus characters (IDs). Optional"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="How many recent timeline events to consider",
    )
    parser.add_argument(
        "--workflow",
        default="world_state_refined",
        help="Workflow name to store refined world state",
    )
    parser.add_argument(
        "--mode",
        default="json_patch",
        choices=["json_patch", "poml_enhance"],
        help="Refinement mode",
    )
    parser.add_argument(
        "--system-prompt-file",
        default="",
        help="Optional path to a system prompt file (for poml_enhance)",
    )
    parser.add_argument(
        "--output-poml",
        default="refined_world_state.poml",
        help="Output path for enhanced POML (poml_enhance mode)",
    )
    ...
    refiner = WorldStateRefiner(
        pipeline.orchestrator, pipeline.poml, manager=pipeline.world_manager
    )
        ...
        json.dumps(
            {
                ...
                "poml_output": (
                    args.output_poml if args.mode == "poml_enhance" else None
                ),
            },
            indent=2,
        )
