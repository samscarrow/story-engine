...
class NarrativePipeline:
    ...
    def __init__(
        ...
        self._profiles = {
            k: {**defaults[k], **conf.get(k, {})} for k in defaults.keys()
        }
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        context if context else "You are a creative narrative designer."
                    ),
                },
...
Scene situation:"""
        ...
        if not situation:
            # Fallback
            situation = (
                f"{beat['name']}: The characters face {beat['purpose'].lower()}."
            )
    ...
    def _calculate_emphasis(
        self, tension: float, characters: List[Dict]
    ) -> Dict[str, str]:
    ...
    async def simulate_character_response(
        self, scene: SceneDescriptor, character: Dict
    ) -> Dict[str, Any]:
...
{{"dialogue": "what you say", "thought": "inner monologue", "action": "physical action"}}"""
    ...
    async def run_full_pipeline(
        self, title: str, premise: str, characters: List[Dict], num_scenes: int = 5
    ):
        ...
        for scene in self.scenes:
            ...
            self.story_output.append({"scene": scene, "responses": scene_responses})
