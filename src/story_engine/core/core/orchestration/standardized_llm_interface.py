...
class StandardizedLLMInterface:
    ...
    async def query(self, standardized_query: StandardizedQuery) -> LLMResponse:
        ...
        try:
            ...
            persona = standardized_query.persona_override or self.QUERY_TO_PERSONA_MAP[standardized_query.query_type]
            ...
            self._update_performance_metrics(query_type_str, response_time, success=True)
            ...
            response_time = time.time() - start_time
            self._update_performance_metrics(query_type_str, response_time, success=False)
    ...
    async def batch_query(
        self, 
        queries: List[StandardizedQuery],
    ...
    def _update_performance_metrics(self, query_type: str, response_time: float, success: bool):
        ...
        self.performance_metrics['average_response_times'][query_type].append(response_time)
        ...
        if query_type not in self.performance_metrics['success_rates']:
            self.performance_metrics['success_rates'][query_type] = {'total': 0, 'successes': 0}
        
        self.performance_metrics['success_rates'][query_type]['total'] += 1
        if success:
            self.performance_metrics['success_rates'][query_type]['successes'] += 1
    
    def _add_to_history(self, query: StandardizedQuery, response: LLMResponse, response_time: float):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        avg_times = {}
        for query_type, times in self.performance_metrics['average_response_times'].items():
            if times:
        ...
        return {
            ...
            'query_type_distribution': dict(self.performance_metrics['query_type_counts']),
