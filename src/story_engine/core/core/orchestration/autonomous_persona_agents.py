...
class TemplateRepository:
    ...
    def generate_template_id(self, context: SimulationContext) -> str:
        ...
        return hashlib.sha256(f"{self.persona_name}:{context_str}".encode()).hexdigest()[:16]
    ...
    async def find_similar_templates(
        ...
        similar_templates.sort(key=lambda tid: (
            self.metadata_cache.get(tid, TemplateMetadata('', '', '', datetime.now(), datetime.now(), 0, 0.0, 0.0, None, [])).usage_count,
            self.metadata_cache.get(tid, TemplateMetadata('', '', '', datetime.now(), datetime.now(), 0, 0.0, 0.0, None, [])).performance_score
        ), reverse=True)
        
        return similar_templates
    
    def _calculate_similarity(self, context: SimulationContext, metadata: TemplateMetadata) -> float:
    ...
    async def update_template_performance(
        ...
        if metadata:
            ...
            await self.store_template(template_id, await self.load_template(template_id), metadata)
...
class AutonomousPersonaAgent(ABC):
    ...
    @abstractmethod
    def extract_simulation_requirements(self, context_data: Dict[str, Any]) -> List[str]:
    ...
    async def run_simulation(
        ...
        try:
            ...
            recursive_results = await self._handle_recursive_simulations(context, result)
            ...
            final_result = {
                ...
                'template_used': template_content[:200] + '...',  # Truncated for logging
    ...
    async def _get_or_create_template(self, context: SimulationContext) -> str:
        ...
        if similar_templates:
            ...
            if template_content:
                logger.info(f"Using existing template {best_template_id} for {self.persona.value}")
                ...
                if await self._should_modify_template(context, template_content):
                    ...
                    await self._store_new_template(new_template_id, template_content, context, best_template_id)
    ...
    async def _execute_simulation(
        ...
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.poml', delete=False) as tmp_file:
            tmp_file.write(template_content)
            tmp_path = Path(tmp_file.name)

        try:
            # Render template with context data
            rendered_prompt = self.poml_engine.render(str(tmp_path), context.context_data)
        finally:
    ...
    async def _evaluate_and_learn(
        ...
        success = bool(
            hasattr(result, 'text') and result.text or 
        ...
        logger.info(f"Template performance updated: {template_id}, success={success}, score={performance_score}")
    ...
    async def _handle_recursive_simulations(
        ...
        if any(keyword in result_text.lower() for keyword in recursive_keywords):
            ...
            logger.info(f"Triggering recursive simulation for {self.persona.value} at depth {context.depth + 1}")
    ...
    async def _generate_meta_template_creation_prompt(self, context: SimulationContext) -> str:
...
class PersonaAgentFactory:
    ...
    def create_all_agents(
        ...
        for persona in cls.AGENT_CLASSES:
            try:
                agents[persona] = cls.create_agent(persona, orchestrator, repository_path)
