...
class UnifiedLLMOrchestrator:
    ...
    def __init__(
        ...
        self.poml = POMLEngine(
            config_file=poml_config.get("config_file") if poml_config else None
        )
        ...
        logger.info(
            f"UnifiedLLMOrchestrator initialized with {len(self.persona_configs)} personas"
        )
    ...
    def _load_persona_configs_from_file(
        self, config_path: str
    ) -> Dict[LLMPersona, PersonaConfig]:
        ...
        try:
            ...
            for persona_name, config_data in raw_configs.get("personas", {}).items():
                try:
                    ...
                    configs[persona_enum] = PersonaConfig(
                        persona=persona_enum, **config_data
                    )
                except ValueError:
                    logger.warning(
                        f"Skipping unknown persona '{persona_name}' in config file."
                    )
            return configs
        except (IOError, ImportError) as e:
            logger.error(
                f"Could not load persona config file '{config_path}': {e}. Using empty config."
            )
            return {}
    ...
    async def generate_with_persona(
        ...
        **kwargs,
        ...
        try:
            ...
            _roles_enabled = str(_os.environ.get("POML_ROLES") or "").lower() in {
                "1",
                "true",
                "yes",
                "on",
    ...
    async def assess_quality(
        ...
        **kwargs,
        ...
        return await self.generate_with_persona(
            ...
            {"content": content, "metrics": metrics, "standards": standards or {}},
            **kwargs,
        )
    ...
    async def batch_generate_with_personas(
        ...
        async def process_request(request: Dict[str, Any]) -> LLMResponse:
            async with semaphore:
                ...
                data = request["data"]
                kwargs = {
                    k: v for k, v in request.items() if k not in ["persona", "data"]
                }
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rates": {
                persona: (
                    successes / self.metrics["persona_usage"][persona]
                    if self.metrics["persona_usage"][persona] > 0
                    else 0.0
                )
...
def create_unified_orchestrator(
    orchestrator: LLMOrchestrator, poml_config: Optional[Dict] = None
) -> UnifiedLLMOrchestrator:
