...
from .autonomous_persona_agents import (
    PersonaAgentFactory
)
...
class SimulationScheduler:
    ...
    def __init__(self, max_concurrent: int = 5):
        ...
        logger.info(f"SimulationScheduler initialized with max_concurrent={max_concurrent}")
    ...
    def add_request(self, request: SimulationRequest):
        ...
        logger.info(f"Added simulation request {request.request_id} with priority {request.priority.value}")
...
class RecursiveSimulationEngine:
    ...
    def __init__(
        ...
        logger.info(f"RecursiveSimulationEngine initialized with {len(self.agents)} agents")
    
    def register_callback(self, name: str, callback: Callable[[SimulationResult], None]):
    ...
    async def submit_simulation(
        ...
        logger.info(f"Submitted simulation {request_id} for persona {persona.value} at depth {depth}")
        
        return request_id
    
    async def get_simulation_result(self, request_id: str) -> Optional[SimulationResult]:
    ...
    async def _execute_simulation(self, request: SimulationRequest):
        ...
        async with self.scheduler.semaphore:
            ...
            try:
                ...
                if not agent:
                    raise ValueError(f"No agent available for persona {request.persona}")
                
                logger.info(f"Executing simulation {request.request_id} with {request.persona.value}")
    ...
    async def _process_recursive_results(
        ...
        for recursive_sim in recursive_data:
            ...
            recursive_request_id = await self.submit_simulation(
                ...
                timeout_seconds=request.timeout_seconds // 2,  # Shorter timeout for recursive
                callbacks=[],
            ...
            while wait_time < max_wait:
                recursive_result = await self.get_simulation_result(recursive_request_id)
                if recursive_result:
                    break
                await asyncio.sleep(1)
                wait_time += 1
    ...
    async def _trigger_callbacks(self, request: SimulationRequest, result: SimulationResult):
