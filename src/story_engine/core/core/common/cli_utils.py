...
def detect_lmstudio_model(endpoint: str) -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            prefer_small = str(os.environ.get("LM_PREFER_SMALL", "")).strip().lower() in {"1", "true", "yes", "on"}
            filtered = filter_models(models, require_text=True, prefer_small=prefer_small)
...
def print_connection_status(config: Dict[str, Any]) -> None:
    ...
    if is_valid:
        ...
        if config['client'] == "lmstudio":
            ...
            print("   💡 Start KoboldCpp with: python koboldcpp.py --model your_model.gguf")
