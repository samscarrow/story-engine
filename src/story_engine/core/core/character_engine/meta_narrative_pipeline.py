...
from story_engine.core.character_engine.character_simulation_engine_v2 import (
    SimulationEngine,
    CharacterState,
    EmotionalState,
    CharacterMemory,
...
class MetaNarrativePipeline:
    ...
    def __init__(
        ...
        character_flags: Optional[Dict[str, Dict[str, Any]]] = None,
        use_iterative_persona: Optional[bool] = None,
    ):
        ...
        self.use_iterative_persona: bool = bool(
            use_iterative_persona
            if use_iterative_persona is not None
            else cfg.get("features", {}).get("strict_persona_mode", False)
        )

    async def craft_scenario_from_world(
        self,
        ...
        brief_md = self.poml.get_world_state_brief_for(
            ws.to_dict(),
        ...
        resp = await self.orchestrator.generate(
            prompt, allow_fallback=True, temperature=0.5, max_tokens=600
        )
    ...
    async def simulate(
        self, character: CharacterState, situations: List[str], runs_per: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    async def simulate_from_world(
        self,
        character: CharacterState,
        runs_per: int = 3,
        focus_chars: Optional[List[str]] = None,
        location: Optional[str] = None,
    ) -> List[Dict[str, Any]]:
        """Craft a scenario from world state, run simulations, then evaluate plausibility against the same world state."""
        scenario = await self.craft_scenario_from_world(
            characters=focus_chars, location=location
        )
        ...
        pov = self.world_manager.pov_subset(
            ws_full,
        ...
        for _ in range(runs_per):
            res = await self.engine.run_simulation(
                character, situation, emphasis="neutral", world_pov=pov_brief
            )
            results.append(res)
        sims = results
    ...
    async def synthesize_meta(
        ...
        resp = await self.orchestrator.generate(
            prompt, allow_fallback=True, temperature=0.6, max_tokens=900
        )
    ...
    async def draft_screenplay(self, meta_outline: str, style: str = "HBO Rome") -> str:
        ...
        resp = await self.orchestrator.generate(
            prompt, allow_fallback=True, temperature=0.8, max_tokens=1100
        )
        return getattr(resp, "text", "") or ""
    ...
    async def evaluate_meta(self, meta_outline: str) -> Dict[str, Any]:
        ...
        resp = await self.orchestrator.generate(
            prompt, allow_fallback=True, temperature=0.3, max_tokens=500
        )
        text = getattr(resp, "text", "") or ""
        # Parse into scores
    ...
    def select_best_throughline(
        self, throughlines: List[Dict[str, Any]]
    ...
    def character_from_dict(d: Dict[str, Any]) -> CharacterState:
        return CharacterState(
            ...
            emotional_state=(
                EmotionalState(**d.get("emotional_state", {}))
                if d.get("emotional_state")
                else EmotionalState()
            ),
            memory=CharacterMemory(
                recent_events=d.get("memory", {}).get("recent_events", [])
            ),
