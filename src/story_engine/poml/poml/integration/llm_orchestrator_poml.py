...
class POMLOrchestrator:
    ...
    def _validate_against_known_schema(
        self, schema_path: str, payload: Dict[str, Any]
    ) -> None:
    ...
    def _enforce_schema_and_clean_json(
        self, template_path: str, raw_text: str
    ) -> Dict[str, Any]:
    ...
    async def generate_with_template_roles(
        self,
        template: Union[str, PromptTemplate],
        data: Dict[str, Any],
        provider: str = None,
        **kwargs,
        ...
        try:
            ...
            template_path = (
                template.value if isinstance(template, PromptTemplate) else template
            )
            self.metrics["template_usage"][template_path] = (
                self.metrics["template_usage"].get(template_path, 0) + 1
            )
            ...
            try:
                ...
                joined = (f"[SYSTEM]\n{prompt_system}\n\n[USER]\n{prompt_user}").strip()
                response = await llm.generate_response(
                    prompt=joined, temperature=temperature, max_tokens=max_tokens
                )
    ...
    async def orchestrate_character_response(
        self, character: Any, situation: str, emphasis: str = "neutral", **kwargs
    ) -> Dict:
        """
        ...
        roles_enabled = str(self.config.get("enable_roles") or "").lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        if not roles_enabled:
            roles_enabled = str(_os.environ.get("POML_ROLES") or "").lower() in {
                "1",
                "true",
                "yes",
                "on",
        ...
        if roles_enabled:
            response = await self.generate_with_template_roles(
                template=PromptTemplate.CHARACTER_RESPONSE, data=data, **kwargs
            )
        else:
    ...
    async def orchestrate_scene_creation(
        self, beat: Dict, characters: List[Dict], previous_context: str = "", **kwargs
    ) -> Dict:
        """
        ...
        if roles_enabled:
            response = await self.generate_with_template_roles(
                template=PromptTemplate.SCENE_CRAFTING, data=data, **kwargs
            )
        else:
            response = await self.generate_with_template(
                template=PromptTemplate.SCENE_CRAFTING, data=data, **kwargs
    ...
    async def orchestrate_dialogue(
        self, character: Dict, scene: Dict, dialogue_context: Dict, **kwargs
    ) -> Dict:
        """
        ...
        if not self.poml.validate_template(dialogue_template):
            ...
            return await self.orchestrate_character_response(
                character=character, situation=scene.get("situation", ""), **kwargs
            )
        ...
        if roles_enabled:
            response = await self.generate_with_template_roles(
                template=dialogue_template, data=data, **kwargs
            )
        else:
            response = await self.generate_with_template(
                template=dialogue_template, data=data, **kwargs
    ...
    async def orchestrate_narrative_sequence(
        self, characters: List[Any], story_arc: Dict, **kwargs
    ) -> List[Dict]:
        """
    ...
    async def orchestrate_group_dynamics(
        self, characters: List[Any], situation: str, group_context: Dict, **kwargs
    ) -> Dict:
        """
        ...
        if not self.poml.validate_template(group_template):
            ...
            for character in characters:
                response = await self.orchestrate_character_response(
                    character=character, situation=situation, **kwargs
                )
        ...
        if roles_enabled:
            response = await self.generate_with_template_roles(
                template=group_template, data=data, **kwargs
            )
        else:
            response = await self.generate_with_template(
                template=group_template, data=data, **kwargs
    ...
    def _parse_scene_elements(self, content: str) -> Dict:
        ...
        """
        elements = {"situation": "", "sensory": {}, "opening_line": ""}
        ...
        for line in lines:
            ...
            if "SCENE SITUATION" in line:
                ...
                if current_section == "situation":
                    ...
                    if "Sight:" in line:
                        elements["sensory"]["sight"] = line.replace(
                            "Sight:", ""
                        ).strip()
                    elif "Sound:" in line:
                        elements["sensory"]["sound"] = line.replace(
                            "Sound:", ""
                        ).strip()
                    elif "Atmosphere:" in line:
                        elements["sensory"]["atmosphere"] = line.replace(
                            "Atmosphere:", ""
                        ).strip()
    ...
    async def batch_generate(
        self, requests: List[Dict], max_concurrent: int = 5
    ) -> List[Dict]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                processed_results.append({"error": str(result), "request": requests[i]})
            else:
...
class OrchestratorMigrationHelper:
    ...
    async def compare_outputs(
        poml_orchestrator: POMLOrchestrator, original_orchestrator: Any, test_data: Dict
    ) -> Dict:
        """
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        providers = {"mock": MockLLM(), "backup": MockLLM()}
