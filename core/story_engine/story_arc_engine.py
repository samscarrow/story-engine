...
class StoryArcEngine:
    ...
    async def call_llm(self, system_prompt: str, user_prompt: str, temperature: float = 0.8) -> Optional[Dict]:
    ...
    def create_story_arc(self, 
        ...
        plot_points = self._generate_plot_points(structure, conflict, characters, target_scenes)
    ...
    def _generate_plot_points(self, structure: StoryStructure, 
                             conflict: ConflictType,
                             characters: List[Dict],
                             target_scenes: int) -> List[PlotPoint]:
        ...
        if structure == StoryStructure.THREE_ACT:
            ...
            plot_points.append(PlotPoint(
                ...
                emotional_target={characters[0]['id']: {"fear": 0.9, "confidence": 0.1}}
            ))
  ...
  "pacing": "slow/moderate/fast"
        ...
        if not details:
            ...
            details = {
                ...
                "character_goals": {char_id: "navigate situation" for char_id in plot_point.required_characters},
        ...
        scene = Scene(
            ...
            characters_present=plot_point.required_characters + plot_point.optional_characters,
    ...
    def _calculate_emphasis(self, plot_point: PlotPoint, 
                           characters: Dict[str, Any]) -> Dict[str, str]:
    ...
    async def generate_full_story(self, arc: StoryArc, 
                                 characters: Dict[str, Any]) -> List[Scene]:
        ...
        for i, plot_point in enumerate(arc.plot_points):
            print(f"🎬 Generating scene {i+1}/{len(arc.plot_points)}: {plot_point.name}")
            ...
            if plot_point.prerequisites:
                ...
                if not completed:
                    print(f"  ⚠️ Prerequisites not met, skipping")
                    continue
            
            # Generate scene
            scene = await self.generate_scene_details(plot_point, characters, self.scenes)
...
class IntegratedStoryEngine:
    ...
    def __init__(self, llm_model: str = "google/gemma-2-27b"):
        ...
        self.simulation_engine = SimulationEngine(llm_provider=self.llm, max_concurrent=1)
    ...
    async def run_story(self, title: str, structure: StoryStructure, 
                       theme: str, conflict: ConflictType,
                       character_defs: List[Dict], 
                       setting: Dict) -> Dict:
        """Run complete story generation and simulation"""
        
        print(f"\n{'='*80}")
        print(f"🎭 INTEGRATED STORY GENERATION & SIMULATION")
        print(f"{'='*80}")
        ...
        print(f"\n{'='*60}")
        print(f"🎬 RUNNING CHARACTER SIMULATIONS")
        print(f"{'='*60}")
                ...
                if char_id in self.characters:
                    ...
                    try:
                        ...
                        if scene.sequence_number > 0:
                            character.memory.add_event(f"Scene {scene.sequence_number-1} completed")
                        
                        # Run simulation
                        result = await self.simulation_engine.run_simulation(
                            character,
                            scene.situation,
                            emphasis=scene.emphasis_per_character.get(char_id, "neutral"),
                            temperature=0.8
                        )
    ...
    def _generate_story_summary(self) -> Dict:
        ...
        summary = {
            "title": self.story_engine.current_arc.title if self.story_engine.current_arc else "Untitled",
    ...
    characters = [
        {
            'id': 'protagonist',
            'name': 'Sarah Chen',
            'backstory': {'role': 'Whistleblower', 'background': 'Former corporate executive'},
