...
class GroupCharacter:
    ...
    trust_levels: Dict[str, float] = field(default_factory=dict)  # character_id: trust (0-1)
    ...
    def calculate_action_weight(self, action: ActionType, situation_tension: float) -> float:
        ...
        if action == ActionType.THREATEN:
            base_weight *= (self.emotional_state["anger"] + self.emotional_state["confidence"]) / 2
...
class GroupDynamicsEngine:
    ...
    def calculate_group_dynamics(self, present_characters: List[str]) -> Dict:
        ...
        return {
            ...
            "dominant_faction": max(factions_present.keys(), 
                                   key=lambda f: sum(self.characters[c].influence_power 
                                                    for c in factions_present[f]))
                              if factions_present else None
        }
    
    async def generate_character_action(self, character: GroupCharacter, 
                                       situation: str, dynamics: Dict, 
                                       previous_actions: List[Dict]) -> Dict:
        ...
        for action in previous_actions[-3:]:  # Last 3 actions
            action_context.append(f"{action['character']}: {action.get('action_type', 'spoke')}")
        
        # Determine likely action type
        action_weights = {}
        for action_type in ActionType:
            weight = character.calculate_action_weight(action_type, dynamics["tension_level"])
...
{{"action_type": "one of: accuse/defend/threaten/plead/conspire/observe/intervene/retreat", "dialogue": "what you say", "thought": "inner thoughts", "gesture": "physical action", "target": "who you're addressing", "intended_effect": "what you hope to achieve", "emotional_shift": {{"anger": 0, "fear": 0, "desperation": 0}}}}"""
    ...
    async def run_complex_scene(self, scene: Dict):
        ...
        ordered_chars = sorted(scene['characters'], 
                ...
                if response:
                    ...
                    if 'emotional_shift' in response:
                        for emotion, shift in response['emotional_shift'].items():
                            if emotion in character.emotional_state and isinstance(shift, (int, float)):
    ...
    def analyze_power_shifts(self):
        ...
        for char_id, character in self.characters.items():
            ...
            print(f"  Allies: {', '.join(character.allies) if character.allies else 'None'}")
            print(f"  Enemies: {', '.join(character.enemies) if character.enemies else 'None'}")
...
def create_expanded_cast() -> Dict[str, GroupCharacter]:
    ...
    characters["pilate"] = GroupCharacter(
        ...
        motivations={"maintain_order": 0.9, "survive_politically": 0.8, "find_truth": 0.4},
        preferred_actions=[ActionType.OBSERVE, ActionType.INTERVENE, ActionType.THREATEN],
        influence_power=0.9
    )
    ...
    characters["peter"] = GroupCharacter(
        ...
        influence_power=0.2,
        emotional_state={"anger": 0.4, "fear": 0.8, "confidence": 0.2, "tension": 0.9, "desperation": 0.7}
    )
    ...
    characters["judas"] = GroupCharacter(
        ...
        influence_power=0.1,
        emotional_state={"anger": 0.2, "fear": 0.9, "confidence": 0.1, "tension": 1.0, "desperation": 1.0}
    )
    ...
    for faction in Faction:
        ...
        if faction_chars:
            ...
            for char in faction_chars:
                print(f"  • {char.name} - {char.role} (Influence: {char.influence_power:.1f})")
    ...
    scenes = [
        {
            ...
            "characters": ["pilate", "caiaphas", "annas", "peter", "crowd_leader", "marcus"]
        },
        {
            "name": "The Bargain",
            ...
            "characters": ["pilate", "caiaphas", "barabbas", "simon_zealot", "crowd_leader", "peter"]
        },
        {
