...
def detect_lmstudio_model(endpoint: str) -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if models:
                # Return the first model (usually the loaded one)
...
def print_connection_status(config: Dict[str, Any]) -> None:
    ...
    if is_valid:
        print(f"   Status: ✅ Connection validated")
    else:
        print(f"   Status: ❌ Connection failed - check if server is running")
        
        if config['client'] == "lmstudio":
            print(f"   💡 Start LMStudio and load a model, then try again")
        elif config['client'] == "koboldcpp":
            print(f"   💡 Start KoboldCpp with: python koboldcpp.py --model your_model.gguf")
